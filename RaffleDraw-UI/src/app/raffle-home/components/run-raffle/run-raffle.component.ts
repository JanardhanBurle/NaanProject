...
export class RunRaffleComponent implements OnInit {
  ...
  showRaffleScreen = false;
  constructor(
    public dialogRef: MatDialogRef<RunRaffleComponent>,
  ...
  ngOnInit() {
    ...
    this.promotionService.getPromotions().subscribe(response => {
      const promos: any = response;
      this.promotionsList = promos.filter(promo => promo.isCompleted);
      if (!this.raffleData) {
  ...
  createRaffleForm(data?: any) {
    this.createRaffle = this.fb.group({
      raffleName: new FormControl({ value: data ? data.raffleName : null, disabled: data ? data.isCompleted : false }, Validators.required),
    ...
    // this.onFormChanges();
    if (data.promotion) {
      this.prizes = data.promotion.prizes;
      this.numberOfWinners = data.promotion.noOfWinners;
    }
    this.showRaffleScreen = true;
  }
  ...
  sendRaffleInfo() {
    ...
    this.raffleService.saveRaffle(newRaffle).subscribe(updatedData => {
